...
describe( 'StatList', () => {
	...
	describe( 'required options', () => {
		let statListFactory = options => new StatList( options );
	...
	describe( 'attributes', function() {
		...
		beforeEach( () => {
			let options = { siteID: 1, statType: 'statsClicks', date: '2014-07-04', period: 'day' };
			statList = new StatList( options );

			let errorOptions = { siteID: 1, statType: 'statsEvents' };
			errorStatList = new StatList( errorOptions );
		} );

		it( 'is always true', () => assert.isTrue( true ) )
	...
	describe( 'object data sets', () => {
		...
		it( 'should set loading false', () => {
			let mockContext = { options: { period: 'day', date: '2014-09-12' } }; // we have to pass in some context to calculate start of period on this call

			let response = statsParser.statsComments.call( mockContext, data.successResponses.statsComments.body );

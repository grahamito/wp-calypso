...
const PostTypeFilter = React.createClass( {
	...
	getNavItems() {
		...
		return map( counts, ( count, status ) => {
			...
			let label;
			switch ( status ) {
				case 'publish': label = this.translate( 'Published', { context: 'Filter label for posts list' } ); break;
				case 'draft': label = this.translate( 'Drafts', { context: 'Filter label for posts list' } ); break;
				case 'future': label = this.translate( 'Scheduled', { context: 'Filter label for posts list' } ); break;
				case 'trash': label = this.translate( 'Trashed', { context: 'Filter label for posts list' } ); break;
			}
